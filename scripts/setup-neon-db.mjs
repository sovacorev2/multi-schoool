...
async function setupNeonDatabase() {
  ...
  try {
    ...
    const examResult = await sql`SELECT COUNT(*) as count FROM exam_types`
    const classResult = await sql`SELECT COUNT(*) as count FROM classes`
    const settingResult = await sql`SELECT COUNT(*) as count FROM admin_settings`

    console.log(`  ✓ Exam types: ${examResult[0].count}`)
    console.log(`  ✓ Classes: ${classResult[0].count}`)
    console.log(`  ✓ Admin settings: ${settingResult[0].count}`)
