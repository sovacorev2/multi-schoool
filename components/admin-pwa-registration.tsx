...
export function AdminPWARegistration({ schoolId, schoolName }: { schoolId?: string; schoolName?: string }) {
  ...
  useEffect(() => {
    // Update manifest link for admin portal
    ...
    if ('serviceWorker' in navigator) {
      navigator.serviceWorker
        .register('/sw.js')
        .then((registration) => {
          console.log('[v0] Admin SW registered:', registration.scope)
        })
        .catch((error) => {
          console.log('[v0] Admin SW registration failed:', error)
        })
    }
    ...
    const handleBeforeInstallPrompt = (e: Event) => {
      console.log('[v0] Admin beforeinstallprompt fired!')
      e.preventDefault()
    ...
    console.log('[v0] Adding admin beforeinstallprompt listener')
