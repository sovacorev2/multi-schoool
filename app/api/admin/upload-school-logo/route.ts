...
export async function POST(request: NextRequest) {
  try {
    ...
    // Store just the filename in database (not the full URL)
    // The full URL will be generated by our serve route
    ...
    // Return the serve route URL instead of the direct blob URL
    const serveUrl = `/api/admin/logo/${encodeURIComponent(filename)}`
    ...
    return NextResponse.json({
      ...
      school: {
        ...school,
        logo_url: serveUrl // Return the serve URL
      },
      logoUrl: serveUrl
    })
  } catch (error) {
